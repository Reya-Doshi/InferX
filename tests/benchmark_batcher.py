...
async def run_benchmark(count: int = 5000) -> None:
    ...
    for i in range(count):
        ...
        requests.append(
            ScheduledRequest(
                request_id=f"r-{i}", tenant_id="t1", priority=1, payload=tokens
            )
        )
    ...
    efficiency = (
        handler.total_actual_tokens / handler.total_padded_tokens
        if handler.total_padded_tokens > 0
        else 0.0
    )
    ...
    print(f"  Time Taken        : {duration:.4f} seconds")
    print(
        f"  Token Savings     : {((handler.total_padded_tokens - total_padded) / handler.total_padded_tokens) * 100:.2f} % fewer tokens padded"
    )
    print("=" * 70 + "\n")
