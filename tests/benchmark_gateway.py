...
async def run_gateway_benchmark(client_count: int = 10, reqs_per_client: int = 200) -> None:
    ...
    tasks = [
        asyncio.create_task(client_worker(manager.host, manager.port, reqs_per_client, payload))
    ...
    p95_us = all_latencies[int(len(all_latencies) * 0.95)] * 1e6 if all_latencies else 0.0
