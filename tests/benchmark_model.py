...
async def run_model_benchmark(count: int = 1000) -> None:
    ...
    m1 = ModelMetadata(
        ...
        estimated_vram_bytes=int(2.5 * 1024 * 1024 * 1024),
    )
    m2 = ModelMetadata(
        model_name="model-B",
        version="v1.0",
    ...
    len(manager.tokenizer.encode(query))

    start_time = time.perf_counter()

    total_generated_tokens = 0
    for _ in range(count):
        await manager.predict("model-A", "v1.0", query)
    ...
    print(
        f"Model Swap (Cache Miss)   : {miss_time_ms:.4f} ms (Includes LRU Eviction & Warmup)"
    )
    print("=" * 70 + "\n")
