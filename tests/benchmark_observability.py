...
async def run_instrumented(count: int, tracer: Tracer, registry: MetricsRegistry) -> float:
    ...
    latency = registry.histogram("test_latency", "Test latency", buckets=[0.1, 0.5, 1.0])
    
    start = time.perf_counter()
    for _ in range(count):
        async with tracer.span("hot_path", attributes={"key": "val"}) as span:
            mock_hot_path_action()
