        ...
        async with self._cond:
            while self.policy.size() == 0:
                if not self._is_active:
                    raise asyncio.CancelledError("Scheduler stopped while waiting for requests.")
                await self._cond.wait()
            
            request = self.policy.pop()
            if request is None:
                raise asyncio.CancelledError("Queue state changed or pop operation yielded None.")
    ...
    async def _aging_worker_loop(self) -> None:
        ...
        while self._is_active:
            try:
                ...
                logger.error(f"Error in scheduler aging loop: {e}", exc_info=True, component="scheduler")
