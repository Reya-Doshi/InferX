...
from inferx.errors.taxonomy import WorkerError
...
class RuntimeSupervisor(IRuntimeSupervisor):
    ...
    async def start(self) -> None:
        ...
        async with self._lock:
            ...
            logger.info(f"Starting runtime supervisor for GPUs: {self.gpus}", component="supervisor")
        ...
        async with self._lock:
            ...
            for worker_id, handle in list(self._workers.items()):
                ...
                if proc.is_alive():
                    ...
                    if proc.is_alive():
                        logger.warning(f"Worker {worker_id} did not exit cleanly. Force killing.", component="supervisor")
                        proc.terminate()
    ...
    async def recover_worker(self, worker_id: str) -> None:
        ...
        async with self._lock:
            ...
            logger.error(f"Worker {worker_id} failure detected. Triggering recovery...", component="supervisor")
            ...
            try:
                ...
                await asyncio.to_thread(self._spawn_worker, worker_id, gpu_id)
                logger.info(f"Worker {worker_id} recovery complete. Restarted successfully.", component="supervisor")
            finally:
    ...
    def _spawn_worker(self, worker_id: str, gpu_id: int) -> None:
        ...
        logger.info(f"Worker {worker_id} spawned (PID: {process.pid})", component="supervisor")
            ...
            try:
                ...
                for handle in workers_snapshot:
                    ...
                    if not proc.is_alive():
                        logger.error(f"Worker {worker_id} process died unexpectedly (Exit Code: {proc.exitcode}).", component="supervisor")
                ...
                logger.error(f"Error in watchdog monitor loop: {e}", exc_info=True, component="supervisor")
