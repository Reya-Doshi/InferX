...
class RuntimeLifecycle(IRuntimeLifecycle):
    ...
    def __init__(self, context: RuntimeContext, supervisor: IRuntimeSupervisor) -> None:
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            logger.info(
                "InferX Runtime Core running. Press Ctrl+C to terminate.",
                component="lifecycle",
            )
            ...
            logger.fatal(
                f"Failed during lifecycle execution loop: {e}",
                exc_info=True,
                component="lifecycle",
            )
        ...
        async with self._shutdown_lock:
            ...
            try:
                ...
                try:
                    ...
                    logger.error(
                        f"Error during supervisor shutdown: {e}",
                        exc_info=True,
                        component="lifecycle",
                    )
                ...
                logger.info(
                    "Graceful shutdown sequence complete.", component="lifecycle"
                )

            except Exception as e:
                logger.error(
                    f"Shutdown sequence encountered failure: {e}",
                    exc_info=True,
                    component="lifecycle",
                )
                try:
        ...
        if sys.platform != "win32":
            ...
            def handle_windows_signal(signum: int, frame: Any) -> None:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to post shutdown task: {e}", component="lifecycle"
                    )
