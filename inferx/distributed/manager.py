...
class ClusterManager:
    ...
    def __init__(
        ...
        self.rpc_server = ClusterRpcServer(
            host=host, port=port, security_token=security_token
        )
        ...
        self.rpc_server.register_handler(
            "request_vote", self.election.handle_request_vote
        )
        ...
        self.rpc_server.register_handler(
            "replicate_state", self.state_manager.handle_replicate_state
        )
    ...
    async def start(self) -> None:
        ...
        self.registry.register_node(self.local_node)
        logger.info(
            f"Cluster Coordinator Node {self.node_id} server active.",
    ...
    async def start_services(self) -> None:
        ...
        logger.info(
            f"Cluster Coordinator Node {self.node_id} background loops active.",
            component="cluster_manager",
        )
    ...
    async def stop(self) -> None:
        ...
        logger.info(
            f"Cluster Coordinator Node {self.node_id} successfully stopped.",
            component="cluster_manager",
        )
    ...
    async def join_cluster(self, seed_host: str, seed_port: int) -> None:
        ...
        """
        logger.info(
            f"Node {self.node_id} attempting to join cluster via seed {seed_host}:{seed_port}...",
            component="cluster_manager",
        )
        ...
        try:
            ...
            logger.info(
                f"Node {self.node_id} successfully joined cluster via seed.",
                component="cluster_manager",
            )

        except Exception as e:
            logger.error(
                f"Failed to join cluster via seed: {e}",
                exc_info=True,
                component="cluster_manager",
            )
            raise
    ...
    async def handle_join_node(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async def replicate_join_gossip():
            peers = [
                p
                for p in self.registry.get_active_nodes()
                if p.node_id not in [self.node_id, new_node.node_id]
            ]
            for p in peers:
                try:
                    await self.rpc_client.call(
                        p.host, p.port, "ping", {"node_info": new_node.model_dump()}
                    )
                except Exception:
                    pass
