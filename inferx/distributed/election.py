...
class LeaderElection(ILeaderElection):
    ...
    async def handle_request_vote(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            if term == self.current_term and (
                ...
                logger.info(
                    f"Node {self.node_id} voted for Candidate {candidate_id} in term {term}",
                    component="election",
                )
    ...
    async def handle_heartbeat(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            return {"term": self.current_term, "success": term >= self.current_term}
    ...
    async def _election_loop(self) -> None:
        ...
        while self._is_running:
            ...
            timeout = (
                random.randint(self.election_min_ms, self.election_max_ms) / 1000.0
            )
    ...
    async def _start_campaign(self) -> None:
        ...
        logger.info(
            f"Node {self.node_id} campaigning for Leader in term {term}",
            component="election",
        )
        ...
        if (
            ...
            logger.error(
                f"Node {self.node_id} ELECTED LEADER for term {term} (Votes: {votes}/{len(self.peers) + 1})",
                component="election",
            )
        ...
        while self._is_running and self.state == "LEADER":
            ...
            async def send_peer_heartbeat(peer: Dict[str, Any]) -> None:
                try:
                    ...
                    logger.error(
                        f"Heartbeat failed to send to peer {peer.get('node_id')}: {type(e).__name__}: {e}",
                        component="election",
                    )
