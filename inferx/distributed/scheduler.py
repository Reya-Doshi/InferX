...
class DistributedScheduler:
    ...
    async def schedule_request(self, request: ScheduledRequest, target_model: str, target_version: str) -> str:
        """
        Schedules a request, deciding between local and remote execution.
        
        Evaluates queue thresholds and remote loads.
        """
        local_node = self.registry.get_node(self.node_id)
        
        # 1. Schedule locally if queue depth is below threshold, or no remote nodes exist
        local_depth = len(self.local_scheduler._queue) if hasattr(self.local_scheduler, "_queue") else 0
        active_peers = [n for n in self.registry.get_active_nodes() if n.node_id != self.node_id]

        if local_depth < self.local_queue_threshold or not active_peers:
            logger.debug(f"Scheduling request {request.request_id} locally. Queue depth: {local_depth}", component="distributed_scheduler")
    ...
    async def handle_remote_task(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        model = params.get("model", "llama")
        version = params.get("version", "latest")

        logger.info(f"Received delegated task {req_id} from remote node.", component="distributed_scheduler")
