...
class ClusterRpcServer(IRpcServer):
    ...
    def __init__(
        self, host: str, port: int, security_token: str = "cluster-secret-key"
    ) -> None:
        self.host = host
        self.port = port
    ...
    async def start(self) -> None:
        ...
        logger.info(
            f"Cluster RPC Server active on {self.host}:{self.port}",
            ...
            while self._is_active:
                ...
                if client_token != self.security_token:
                    resp = {
                        ...
                        "error": {
                            "code": -32001,
                            "message": "Unauthorized cluster token.",
                        },
                        "id": req_id,
                    }
                ...
                if method in self._handlers:
                    try:
                        ...
                        resp = {
                            ...
                            "error": {
                                "code": -32603,
                                "message": f"Internal RPC Error: {e}",
                            },
                            "id": req_id,
                        }
                else:
...
class ClusterRpcClient(IRpcClient):
    ...
    def __init__(
        self, security_token: str = "cluster-secret-key", timeout_sec: float = 3.0
    ) -> None:
        self.security_token = security_token
    ...
    async def call(
        ...
        req = {"jsonrpc": "2.0", "method": method, "params": params_copy, "id": req_id}
        ...
        try:
            ...
            raise ConnectionError(
                f"Failed to connect to cluster node {host}:{port}: {e}"
            )
