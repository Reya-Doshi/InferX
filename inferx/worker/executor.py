    ...
    def __init__(self, stream_id: int) -> None:
        ...
        if self.api_key and not self.is_testing:
            try:
                ...
                logger.error(
                    f"Failed to initialize GenAI client: {e}", component="cuda_stream"
                )

    async def execute_task(
        self, request: ScheduledRequest, execution_time_ms: float = 5.0
    ) -> bytes:
        """
...
class BatchExecutor:
    ...
    async def _run_with_timeout(
        self, stream: CudaStream, req: ScheduledRequest, execution_time_ms: float
    ) -> bytes:
        ...
        try:
            return await asyncio.wait_for(
                stream.execute_task(req, execution_time_ms), timeout=timeout_sec
            )
