...
class DeploymentController(IDeploymentController):
    ...
    def __init__(self, initial_replicas: int = 3, initial_version: str = "v1.0") -> None:
    ...
    async def start_rolling_update(
        ...
        old_instances = [inst for inst in self.instances if inst["version"] != target_version]
    ...
    async def start_canary_deployment(
        ...
        logger.info(f"Initiating canary deployment for version {target_version} (Weight: {canary_weight_percent}%)")
    ...
    async def execute_canary_rollback(self, target_version: str) -> None:
        ...
        logger.warning(f"Error metrics exceeded limits. Rolling back canary version {target_version}")
        # Remove any instance running target_version
        self.instances = [inst for inst in self.instances if inst["version"] != target_version]
        self._state = "STABLE"
        self._canary_weight = 0
